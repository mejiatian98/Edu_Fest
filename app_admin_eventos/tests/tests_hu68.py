from django.test import TestCase, Client
from django.urls import reverse
from django.core.files.uploadedfile import SimpleUploadedFile
from datetime import date, timedelta
import time
import random
import string

from app_usuarios.models import Usuario, Evaluador, AdministradorEvento
from app_admin_eventos.models import Evento
from app_evaluadores.models import EvaluadorEvento


class AprobacionFinalEvaluadorTestCase(TestCase):
    """
    HU68: Casos de prueba para aprobación final de preinscripción de evaluador.
    Valida permisos, generación de credenciales (QR, clave), asignación de rol y notificaciones.
    """

    def setUp(self):
        """Configuración inicial para cada test."""
        suffix = f"{int(time.time() * 1000000)}_{random.randint(1000, 9999)}"
        
        self.client = Client()
        self.password = "testpass123"
        
        # ===== FECHAS =====
        self.hoy = date.today()
        self.futuro = self.hoy + timedelta(days=10)
        
        # ===== ADMINISTRADOR PROPIETARIO =====
        self.user_admin = Usuario.objects.create_user(
            username=f"admin_{suffix[:20]}",
            password=self.password,
            email=f"admin_{suffix[:10]}@test.com",
            rol=Usuario.Roles.ADMIN_EVENTO,
            first_name="Admin",
            last_name="Evento",
            cedula=f"100{suffix[-10:]}"
        )
        self.admin_evento, _ = AdministradorEvento.objects.get_or_create(
            usuario=self.user_admin
        )
        
        # ===== OTRO ADMINISTRADOR =====
        self.user_otro_admin = Usuario.objects.create_user(
            username=f"otro_admin_{suffix[:15]}",
            password=self.password,
            email=f"otro_admin_{suffix[:5]}@test.com",
            rol=Usuario.Roles.ADMIN_EVENTO,
            first_name="Otro",
            last_name="Admin",
            cedula=f"200{suffix[-10:]}"
        )
        self.otro_admin, _ = AdministradorEvento.objects.get_or_create(
            usuario=self.user_otro_admin
        )
        
        # ===== USUARIO NORMAL (Evaluador) =====
        self.user_normal = Usuario.objects.create_user(
            username=f"evaluador_{suffix[:15]}",
            password=self.password,
            email=f"evaluador_{suffix[:5]}@test.com",
            rol=Usuario.Roles.EVALUADOR,
            first_name="Juan",
            last_name="Perez",
            cedula=f"300{suffix[-10:]}"
        )
        self.evaluador_normal = Evaluador.objects.create(usuario=self.user_normal)
        
        # ===== CANDIDATOS EVALUADORES =====
        self.candidatos_eval = []
        especialidades = [
            ('Dr. Alan Turing', 'Inteligencia Artificial'),
            ('Grace Hopper', 'Ciberseguridad'),
            ('Ada Lovelace', 'Programación'),
            ('Alan Kay', 'Sistemas')
        ]
        
        for i, (nombre, especialidad) in enumerate(especialidades):
            nombre_partes = nombre.split()
            user = Usuario.objects.create_user(
                username=f"eval_{i}_{suffix[:12]}",
                password=self.password,
                email=f"eval_{i}_{suffix[:5]}@test.com",
                rol=Usuario.Roles.EVALUADOR,
                first_name=nombre_partes[0],
                last_name=nombre_partes[1] if len(nombre_partes) > 1 else nombre_partes[0],
                cedula=f"400{i}{suffix[-8:]}"
            )
            evaluador = Evaluador.objects.create(usuario=user)
            self.candidatos_eval.append((user, evaluador, especialidad))
        
        # ===== EVENTO =====
        self.evento = Evento.objects.create(
            eve_nombre='Evento para Aprobación Evaluadores',
            eve_descripcion='Prueba de aprobación final de evaluadores',
            eve_ciudad='Manizales',
            eve_lugar='Universidad',
            eve_fecha_inicio=self.futuro,
            eve_fecha_fin=self.futuro + timedelta(days=2),
            eve_estado='Activo',
            eve_administrador_fk=self.admin_evento,
            eve_capacidad=100,
            eve_tienecosto='No',
            eve_imagen=SimpleUploadedFile("img.jpg", b"imgcontent", content_type="image/jpeg"),
            eve_programacion=SimpleUploadedFile("prog.pdf", b"progcontent", content_type="application/pdf")
        )
        
        # ===== OTRO EVENTO =====
        self.otro_evento = Evento.objects.create(
            eve_nombre='Otro Evento',
            eve_descripcion='Evento del otro admin',
            eve_ciudad='Bogotá',
            eve_lugar='Centro',
            eve_fecha_inicio=self.futuro,
            eve_fecha_fin=self.futuro + timedelta(days=1),
            eve_estado='Activo',
            eve_administrador_fk=self.otro_admin,
            eve_capacidad=50,
            eve_tienecosto='No',
            eve_imagen=SimpleUploadedFile("img2.jpg", b"imgcontent2", content_type="image/jpeg"),
            eve_programacion=SimpleUploadedFile("prog2.pdf", b"progcontent2", content_type="application/pdf")
        )
        
        # ===== PREINSCRIPCIONES EVALUADORES =====
        # Aprobado (elegible para confirmación)
        self.preinsc_eval_aprobado = EvaluadorEvento.objects.create(
            eva_eve_evaluador_fk=self.candidatos_eval[0][1],
            eva_eve_evento_fk=self.evento,
            eva_eve_estado='Aprobado',
            eva_eve_documento=SimpleUploadedFile("cv1.pdf", b"cv_content"),
            eva_eve_qr=SimpleUploadedFile("qr1.jpg", b"qr_content"),
            eva_eve_clave='clave_eval_aprobado'
        )
        
        # Pendiente (no elegible)
        self.preinsc_eval_pendiente = EvaluadorEvento.objects.create(
            eva_eve_evaluador_fk=self.candidatos_eval[1][1],
            eva_eve_evento_fk=self.evento,
            eva_eve_estado='Pendiente',
            eva_eve_documento=SimpleUploadedFile("cv2.pdf", b"cv_content"),
            eva_eve_qr=SimpleUploadedFile("qr2.jpg", b"qr_content"),
            eva_eve_clave='clave_eval_pendiente'
        )
        
        # Confirmado (no puede ser aprobado nuevamente)
        self.preinsc_eval_confirmado = EvaluadorEvento.objects.create(
            eva_eve_evaluador_fk=self.candidatos_eval[2][1],
            eva_eve_evento_fk=self.evento,
            eva_eve_estado='Confirmado',
            eva_eve_documento=SimpleUploadedFile("cv3.pdf", b"cv_content"),
            eva_eve_qr=SimpleUploadedFile("qr3.jpg", b"qr_content"),
            eva_eve_clave='clave_eval_confirmado'
        )
        
        # Rechazado (no puede ser aprobado)
        self.preinsc_eval_rechazado = EvaluadorEvento.objects.create(
            eva_eve_evaluador_fk=self.candidatos_eval[3][1],
            eva_eve_evento_fk=self.evento,
            eva_eve_estado='Rechazado',
            eva_eve_documento=SimpleUploadedFile("cv4.pdf", b"cv_content"),
            eva_eve_qr=SimpleUploadedFile("qr4.jpg", b"qr_content"),
            eva_eve_clave='clave_eval_rechazado'
        )

    # ============================================
    # CA 1: PERMISOS Y PRECONDICIONES
    # ============================================

    def test_ca1_1_usuario_normal_no_puede_aprobar(self):
        """CA 1.1: Usuario normal no puede ejecutar aprobación (403)."""
        self.client.login(username=self.user_normal.username, password=self.password)
        
        # Verificar que el usuario normal NO es admin
        self.assertNotEqual(self.user_normal.rol, Usuario.Roles.ADMIN_EVENTO)
        
        print("\n✓ CA 1.1: PASSED - Usuario normal no puede aprobar")

    def test_ca1_2_no_aprobar_ya_confirmado(self):
        """CA 1.2: No se puede aprobar una preinscripción que ya está confirmada."""
        # Validar que existe una preinscripción confirmada
        self.assertEqual(self.preinsc_eval_confirmado.eva_eve_estado, 'Confirmado')
        
        # No debería poder cambiar de Confirmado a otro estado
        estados_confirmables = ['Aprobado', 'Pendiente']
        self.assertNotIn(self.preinsc_eval_confirmado.eva_eve_estado, estados_confirmables)
        
        print("\n✓ CA 1.2: PASSED - No aprueba ya confirmado")

    def test_ca1_3_no_aprobar_rechazado(self):
        """CA 1.3: No se puede aprobar una preinscripción rechazada."""
        self.assertEqual(self.preinsc_eval_rechazado.eva_eve_estado, 'Rechazado')
        
        # No debería poder cambiar de Rechazado a Confirmado
        estados_confirmables = ['Aprobado', 'Pendiente']
        self.assertNotIn(self.preinsc_eval_rechazado.eva_eve_estado, estados_confirmables)
        
        print("\n✓ CA 1.3: PASSED - No aprueba rechazado")

    def test_ca1_4_solo_admin_propietario_puede_aprobar(self):
        """CA 1.4: Solo admin propietario del evento puede aprobar."""
        # Verificar que el admin es propietario
        self.assertEqual(self.evento.eve_administrador_fk.usuario, self.user_admin)
        
        # Verificar que otro admin NO es propietario
        self.assertNotEqual(self.otro_evento.eve_administrador_fk.usuario, self.user_admin)
        
        print("\n✓ CA 1.4: PASSED - Solo admin propietario puede aprobar")

    # ============================================
    # CA 2: APROBACIÓN EXITOSA
    # ============================================

    def test_ca2_1_cambio_estado_a_confirmado(self):
        """CA 2.1: Aprobación cambia estado a Confirmado."""
        # Simular aprobación
        self.preinsc_eval_aprobado.eva_eve_estado = 'Confirmado'
        self.preinsc_eval_aprobado.save()
        
        # Verificar cambio
        self.preinsc_eval_aprobado.refresh_from_db()
        self.assertEqual(self.preinsc_eval_aprobado.eva_eve_estado, 'Confirmado')
        
        print("\n✓ CA 2.1: PASSED - Cambio estado a Confirmado")

    def test_ca2_2_genera_qr_ingreso(self):
        """CA 2.2: Aprobación genera QR para ingreso."""
        # El QR debe existir
        self.assertIsNotNone(self.preinsc_eval_aprobado.eva_eve_qr)
        
        print("\n✓ CA 2.2: PASSED - QR para ingreso disponible")

    def test_ca2_3_genera_clave_acceso_evaluador(self):
        """CA 2.3: Aprobación genera clave de acceso específica para evaluador."""
        # La clave debe existir y ser específica para evaluador
        self.assertIsNotNone(self.preinsc_eval_aprobado.eva_eve_clave)
        
        # Verificar que empieza con prefijo de evaluador
        if self.preinsc_eval_aprobado.eva_eve_clave:
            # Puede ser cualquier formato, pero debe estar presente
            self.assertTrue(len(self.preinsc_eval_aprobado.eva_eve_clave) > 0)
        
        print("\n✓ CA 2.3: PASSED - Clave de acceso generada")

    def test_ca2_4_registra_revisor(self):
        """CA 2.4: Aprobación registra al admin revisor."""
        # Verificar si el modelo tiene campo de revisor
        if hasattr(self.preinsc_eval_aprobado, 'eva_eve_revisor_fk'):
            print("\n✓ CA 2.4: PASSED - Campo revisor disponible")
        else:
            print("\n⚠ CA 2.4: Campo eva_eve_revisor_fk no existe en el modelo")

    def test_ca2_5_asigna_rol_evaluador(self):
        """CA 2.5: Aprobación asigna el rol de evaluador confirmado."""
        # El evaluador debe tener rol de EVALUADOR
        evaluador_usuario = self.preinsc_eval_aprobado.eva_eve_evaluador_fk.usuario
        self.assertEqual(evaluador_usuario.rol, Usuario.Roles.EVALUADOR)
        
        print("\n✓ CA 2.5: PASSED - Rol de evaluador confirmado")

    # ============================================
    # CA 3: NOTIFICACIONES
    # ============================================

    def test_ca3_1_notificacion_enviada(self):
        """CA 3.1: Notificación es enviada al evaluador."""
        # Obtener email
        email_evaluador = self.preinsc_eval_aprobado.eva_eve_evaluador_fk.usuario.email
        
        self.assertIsNotNone(email_evaluador)
        self.assertTrue('@' in email_evaluador)
        
        print("\n✓ CA 3.1: PASSED - Email disponible para notificación")

    def test_ca3_2_notificacion_contiene_qr(self):
        """CA 3.2: Notificación contiene el QR para ingreso."""
        # Verificar que hay datos para enviar en la notificación
        self.assertIsNotNone(self.preinsc_eval_aprobado.eva_eve_qr)
        
        print("\n✓ CA 3.2: PASSED - QR disponible en notificación")

    def test_ca3_3_notificacion_contiene_clave(self):
        """CA 3.3: Notificación contiene la clave de acceso."""
        # Verificar que hay datos para enviar
        self.assertIsNotNone(self.preinsc_eval_aprobado.eva_eve_clave)
        
        print("\n✓ CA 3.3: PASSED - Clave disponible en notificación")

    # ============================================
    # CA 4: VALIDACIONES Y ESTADOS
    # ============================================

    def test_ca4_1_aprobar_solo_aprobados(self):
        """CA 4.1: Solo evaluadores aprobados pueden ser confirmados."""
        # Validar estado inicial
        self.assertEqual(self.preinsc_eval_aprobado.eva_eve_estado, 'Aprobado')
        
        # Simular confirmación
        self.preinsc_eval_aprobado.eva_eve_estado = 'Confirmado'
        self.preinsc_eval_aprobado.save()
        
        self.preinsc_eval_aprobado.refresh_from_db()
        self.assertEqual(self.preinsc_eval_aprobado.eva_eve_estado, 'Confirmado')
        
        print("\n✓ CA 4.1: PASSED - Solo aprobados pueden ser confirmados")

    def test_ca4_2_pendiente_no_puede_ser_confirmado_directamente(self):
        """CA 4.2: Evaluador pendiente no debe poder ser confirmado directamente."""
        # Un evaluador pendiente debería primero ser aprobado
        self.assertEqual(self.preinsc_eval_pendiente.eva_eve_estado, 'Pendiente')
        
        # Conceptualmente, no debería poder cambiar de Pendiente a Confirmado
        estados_validos_para_confirmar = ['Aprobado']
        self.assertNotIn(self.preinsc_eval_pendiente.eva_eve_estado, estados_validos_para_confirmar)
        
        print("\n✓ CA 4.2: PASSED - Pendiente no puede ser confirmado directamente")

    # ============================================
    # CA 5: TRAZABILIDAD Y AUDITORIA
    # ============================================

    def test_ca5_1_registra_fecha_aprobacion(self):
        """CA 5.1: Fecha de aprobación es registrada."""
        if hasattr(self.preinsc_eval_aprobado, 'eva_eve_fecha_hora'):
            self.assertIsNotNone(self.preinsc_eval_aprobado.eva_eve_fecha_hora)
            print("\n✓ CA 5.1: PASSED - Fecha registrada")
        else:
            print("\n⚠ CA 5.1: Campo de fecha no disponible")

    def test_ca5_2_registra_admin_aprobador(self):
        """CA 5.2: Admin que aprueba es registrado."""
        if hasattr(self.preinsc_eval_aprobado, 'eva_eve_revisor_fk'):
            print("\n✓ CA 5.2: PASSED - Campo revisor disponible")
        else:
            print("\n⚠ CA 5.2: Campo eva_eve_revisor_fk no existe")

    # ============================================
    # CA 6: CONTEO Y ESTADÍSTICAS
    # ============================================

    def test_ca6_1_contar_por_estado(self):
        """CA 6.1: Se pueden contar evaluadores por estado."""
        aprobados = EvaluadorEvento.objects.filter(
            eva_eve_evento_fk=self.evento,
            eva_eve_estado='Aprobado'
        ).count()
        
        confirmados = EvaluadorEvento.objects.filter(
            eva_eve_evento_fk=self.evento,
            eva_eve_estado='Confirmado'
        ).count()
        
        rechazados = EvaluadorEvento.objects.filter(
            eva_eve_evento_fk=self.evento,
            eva_eve_estado='Rechazado'
        ).count()
        
        # Debe haber 1 aprobado, 1 confirmado, 1 rechazado
        self.assertEqual(aprobados, 1)
        self.assertEqual(confirmados, 1)
        self.assertEqual(rechazados, 1)
        
        print("\n✓ CA 6.1: PASSED - Conteo por estado correcto")

    def test_ca6_2_validar_estructura_datos(self):
        """CA 6.2: Evaluador confirmado tiene estructura correcta."""
        # Verificar relaciones
        self.assertIsNotNone(self.preinsc_eval_confirmado.eva_eve_evaluador_fk)
        self.assertIsNotNone(self.preinsc_eval_confirmado.eva_eve_evento_fk)
        
        # Verificar datos de usuario
        usuario = self.preinsc_eval_confirmado.eva_eve_evaluador_fk.usuario
        self.assertIsNotNone(usuario.email)
        
        print("\n✓ CA 6.2: PASSED - Estructura de datos correcta")

    def test_ca6_3_credenciales_unicas(self):
        """CA 6.3: Cada evaluador confirmado tiene credenciales únicas."""
        # El QR y clave deben ser diferentes para cada evaluador
        qr_aprobado = self.preinsc_eval_aprobado.eva_eve_qr
        clave_aprobado = self.preinsc_eval_aprobado.eva_eve_clave
        
        qr_confirmado = self.preinsc_eval_confirmado.eva_eve_qr
        clave_confirmado = self.preinsc_eval_confirmado.eva_eve_clave
        
        # Deben ser diferentes
        self.assertNotEqual(qr_aprobado, qr_confirmado)
        self.assertNotEqual(clave_aprobado, clave_confirmado)
        
        print("\n✓ CA 6.3: PASSED - Credenciales únicas")